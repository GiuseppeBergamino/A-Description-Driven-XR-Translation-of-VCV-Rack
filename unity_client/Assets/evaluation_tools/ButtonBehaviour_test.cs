...
public class ButtonBehaviour_test : MonoBehaviour
{
    ...
    [SerializeField] private Color pressSendColor = new Color(0.0f, 1.0f, 0.0f);
    [SerializeField] private float pressFlashDuration = 0.10f;
    ...
    private float pressFlashUntil = -1f;
    ...
    private void HandleLocalPress()
    {
        ...
        {
            SendParamSet(1f, markPressSend: true);
        }
        else
        {
            toggleState = !toggleState;
            remoteNormalizedValue = toggleState ? 1f : 0f;
            SendParamSet(remoteNormalizedValue, markPressSend: true);
        }
    }
    ...
    private void HandleLocalRelease()
    {
        ...
        if (resolvedMode == ButtonMode.Momentary)
        {
            ...
            SendParamSet(0f, markPressSend: false);
        }
    }

    private bool SendParamSet(float normalizedValue, bool markPressSend = false)
    {
    ...
    private void TriggerPressSendFlash()
    {
        pressFlashUntil = Time.unscaledTime + pressFlashDuration;
        ApplyButtonColor(pressSendColor);
    }

    private void ApplyButtonColor(Color currentColor)
    {
    ...
    {
        Color currentColor = Time.unscaledTime < pressFlashUntil
            ? pressSendColor
            : idleColor;

        ApplyButtonColor(currentColor);
    }
    }